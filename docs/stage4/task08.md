...
## 代码复核更新（2026-09-07，R06）

- 删除设备在配置写盘失败（`WRITE_FAILED`）或清理/异常失败分支撤销删除标记后，现在会补偿连接意图：
  - 删除前为自动在线（Online/Degraded/Connecting/ReconnectPending 且非手动断开）的设备补排登录，登录成功自动布防，避免一次失败的删除让设备永久离线。
  - 删除前为手动断开的设备恢复 `ManualDisconnected` 语义，不会被错误重连。
  - 补偿在 `SetDeleting(false)` 之后执行；再次删除成功后不残留旧补偿效果（同设备队列保序，删除清理随后登出）。

## 验收复核更新（2026-09-07，F02）

- 删除失败的自动在线补偿改走统一重连状态机（`EnsureReconnectScheduled`）：标记 `ReconnectPending` 并安排带完成观察的延迟重连任务。补偿登录在设备队列中过期或投递被拒后仍会被重新安排/由健康检查自愈兜底，不再出现"补偿一次失败即永久离线"。

## 第三轮验收更新（2026-09-07，G2）

- 删除失败补偿按清理后的真实资源状态选择动作：SDK 会话仍存活（清理在登出前失败）时不改状态强制重登——保持在线、按需重新布防，会话继续由既有清理流程持有；会话已释放（登出失败经 StaleSdkUserId 移交统一清理，或完全登出后写盘失败）才走统一重连状态机。三条路径均不会产生无法追踪的旧会话。
