...
## 代码复核更新（2026-09-07，R01）

- 延迟重连任务被设备队列接受后，调度器会挂接完成观察回调；登录任务在队列中超过执行期限（执行前过期、委托从未运行）时，由生命周期重新安排重连，设备不再停留在 `ReconnectPending` 无人补救。
- `ScheduleReconnect` 现在检查延迟队列 `Schedule` 返回值，调度被拒绝时记录错误，等待健康检查自愈兜底。
- 健康检查后台任务新增自愈扫描：`ReconnectPending` 且 `NextReconnectAt` 超过宽限期（`DeviceLifecycleOptions.ReconnectSelfHealGraceMs`，默认 60 秒）仍无进展的设备会幂等补排重连（taskKey 合并去重，不产生重复任务）。
- `DeviceTaskResult` 新增 `ExpiredBeforeExecution` 标记，用于区分"执行前过期"与"执行中超时"。
