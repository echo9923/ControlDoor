...
实现 `FaceEventRepository.InsertEvent`，把标准 `AcsFaceEvent` 写入现有 `dbo.attendance_gate_v2` 表，并依赖 `id` 唯一索引保证实时事件和历史补偿不会重复入库。
...
| 历史补偿补到实时已入库事件 | 命中唯一键，视为成功处理。 |
| 历史补偿重复执行 | 命中唯一键，继续推进批次。 |
| EventId 生成碰撞 | 记录 `Error`，人工排查；实现上仍遵守唯一键。 |

重复事件不应当作为失败阻塞历史补偿 checkpoint 前移。

## 事务边界

单条事件入库不需要长事务。历史批量补偿可以按事件逐条插入并收集结果，只有在整批解析和入库处理完成后再更新 checkpoint。

| 操作 | 事务建议 |
| --- | --- |
| 单条 insert | 单命令即可。 |
| 图片保存 | 在数据库 insert 前完成；insert 重复时可保留已写图片或跳过保存。 |
| checkpoint 更新 | 与批次完成结果分开处理。 |

为了避免重复事件反复写图片，历史补偿和实时事件可先尝试查重再保存图片；但最终仍以唯一键作为准。
...
| 数据库连接失败 | 返回 RetryableFailure，实时事件记录错误，历史补偿不前移 checkpoint。 |
...
| 不修改唯一索引 | 现有表结构冻结。 |
| 不删除事件 | 阶段 7 只写入。 |
| 不把历史补偿状态写入补偿表 | 历史事件补偿使用 checkpoint，不使用 `device_operation_retry_states`。 |
