...
## 代码复核更新（2026-09-07，R05/R10/R11）

- 抓拍落盘幂等：同一事件重试时目标文件已存在则直接复用路径（文件名含全局唯一 EventId，temp+Move 原子写保证已存在文件完整），不再生成无数据库引用的孤立图片。
- 批量查询分块：查重与昵称批量查询的 IN 参数按 1000 一块分块执行，避免大批次超过 SQL Server 每命令 2100 参数上限导致整批失败。
- 批量路径统一超时：批量插入、查重、昵称查询的 SqlCommand 均设置 `Database.CommandTimeoutSeconds`（与单条路径一致）。注意 CommandTimeout 是单命令期限，批量事务总时长为各条命令期限之和。
