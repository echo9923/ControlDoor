...
| 表 | 验证 |
| --- | --- |
| `devices` | 遗留兼容表如存在则只验证结构不变；当前设备加载、插入、删除改由 `Configuration/devices.json` 验证。 |
...
`dbo.devices` 仅作为遗留兼容表保留在初始化脚本和数据库文档中；当前版本的设备主数据来自 `Configuration/devices.json`，运行前检查不再要求该表可读。
